    ...
    if (empty($name) || empty($sku) || $price <= 0) {
        ...
        if ($existing) {
            ...
            if (!empty($_FILES['image']['name'])) {
                ...
                $image = uploadImage($_FILES['image'], $targetDir);
            }
            ...
            execute(
                "INSERT INTO products (name, sku, brand, category_id, price, old_price, image, stock_status, stock_qty, moq, specs, description, features, tags, discount_percentage, featured, warranty) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                [$name, $sku, $brand, $category_id, $price, $old_price, $image, $stock_status, $stock_qty, $moq, $specs, $description, $features, $tags, $discount, $featured, $warranty ?: null]
            );

            $newId = fetchOne("SELECT MAX(id) as id FROM products")['id'];
            if (!empty($_FILES['extra_images']['name'][0])) {
                ...
                foreach ($_FILES['extra_images']['tmp_name'] as $k => $tmpName) {
                    ...
                    if ($uploaded) {
                        execute("INSERT INTO product_images (product_id, image, sort_order) VALUES (?, ?, ?)", [$newId, $uploaded, $k]);
                    }
                }
            }

            $success = 'Product "' . htmlspecialchars($name) . '" has been added successfully.';
        }
    }
}
?>
...
<head>
    ...
    <title>Add Product — BN-Infrastructure Admin</title>
...
<body>
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="products-add.php" class="active"><i class="fas fa-plus-circle"></i> <span>Add Product</span></a>
    ...
    <div class="main">
        <div class="main-header">
            ...
            <p>Create a new product listing for the catalog</p>
        </div>

        <?php if ($success): ?>
        <div class="alert alert-success"><i class="fas fa-check-circle"></i> <?php echo $success; ?> <a href="products-add.php" style="color:#059669;font-weight:600;">Add another</a> or <a href="products.php" style="color:#059669;font-weight:600;">view all products</a>.</div>
        <?php endif; ?>
        <?php if ($error): ?>
                ...
                <form method="POST" enctype="multipart/form-data">
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" placeholder="e.g. Cisco Catalyst 2960-X Switch" required>
                        </div>
                        <div class="form-group">
                            <label>SKU <span class="required">*</span></label>
                            <input type="text" name="sku" placeholder="e.g. WS-C2960X-48FPD-L" required>
                            <div class="help">Unique product identifier</div>
                        </div>
                        <div class="form-group">
                            <label>Brand</label>
                            <input type="text" name="brand" placeholder="e.g. Cisco, Ubiquiti, MikroTik">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="price" placeholder="e.g. 7200000" min="0" step="0.01" required>
                        </div>
                        <div class="form-group">
                            <label>Old/Original Price (TSh)</label>
                            <input type="number" name="old_price" placeholder="e.g. 8200000" min="0" step="0.01">
                            <div class="help">Leave empty if no discount</div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="help">Accepted: JPG, PNG, WebP. Place in Homepage/</div>
                        </div>
                        <div class="form-group">
                            <label>Additional Images</label>
                            <input type="file" name="extra_images[]" accept="image/*" multiple>
                            <div class="help">Upload multiple gallery images</div>
                        </div>
                        <div class="form-group">
                            <label>Warranty</label>
                            <input type="text" name="warranty" placeholder="e.g. 1 Year Manufacturer Warranty">
                            <div class="help">Warranty period and details</div>
                        </div>
                        <div class="form-group full">
                            <label>Short Specs</label>
                            <input type="text" name="specs" placeholder="e.g. 48-port, Gigabit, PoE+, 2x SFP+ Uplinks">
                        </div>
                        <div class="form-group full">
                            <label>Tags (comma separated)</label>
                            <input type="text" name="tags" placeholder="e.g. Managed, PoE+, Gigabit">
                            <div class="help">Feature badges shown on product cards</div>
                        </div>
                        <div class="form-group full">
                            <label>Description</label>
                            <textarea name="description" placeholder="Full product description..."></textarea>
                        </div>
                        <div class="form-group full">
                        ...
                        <div class="form-group full">
                            <div class="form-row-checkbox">
                                ...
                                <label for="featured">Featured Product (shown on homepage)</label>
                            </div>
                        </div>
                    </div>
